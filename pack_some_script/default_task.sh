...
sh -x /pss/pack_some_script/scripts_update.sh
...
if [ 0"$CUSTOM_SHELL_FILE" = "0" ]; then
    ...
    if expr "$CUSTOM_SHELL_FILE" : 'http.*' &>/dev/null; then
        ...
        echo "下载完成，开始执行..."
        sh -x /pss/pack_some_script/pss_shell_mod.sh
...
cat /pss/pack_some_script/docker_entrypoint.sh >/usr/local/bin/docker_entrypoint.sh
