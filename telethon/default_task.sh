...
defaultListFile="/pss/telethon/$DEFAULT_LIST_FILE"
echo "默认文件定时任务文件路径为 ${defaultListFile}"
if [ $CUSTOM_LIST_FILE ]; then
    customListFile="/pss/telethon/$CUSTOM_LIST_FILE"
    echo "自定义定时任务文件路径为 ${customListFile}"
fi
mergedListFile="/pss/telethon/merged_list_file.sh"
...
if [ $(grep -c "docker_entrypoint.sh" $mergedListFile) -eq '0' ]; then
    ...
    echo "0 */1 * * * sleep 10 && docker_entrypoint.sh >> /logs/default_task.log 2>&1" >>$mergedListFile
else
